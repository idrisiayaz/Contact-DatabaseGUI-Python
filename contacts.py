from tkinter import Tk,Button,PhotoImage,Label,LabelFrame,E,W,N,S,Entry,END,StringVar,Scrollbar,Toplevel
...
class Contacts:
    ...
    def create_left_icon(self):
        photo = PhotoImage(file='C:\\Users\\idris\\python\\contacts\\icons\\bro (2).png')
        label = Label(image=photo)
        label.image = photo
        label.grid(row=0, column=0)

    def create_label_frame(self):
        labelframe = LabelFrame(self.root, text='Create New Contact',bg="sky blue",fg="black",font="helvetica 10")
        labelframe.grid(row=0, column=1, padx=8, pady=8, sticky='ew')
        Label(labelframe, text='Patient Name:',bg="green",fg="white").grid(row=1, column=1, sticky=W, pady=2,padx=15)
        self.namefield = Entry(labelframe)
        self.namefield.grid(row=1, column=2, sticky=W, padx=5, pady=2)
        Label(labelframe, text='Phone Number:',bg="brown",fg="white").grid(row=2, column=1, sticky=W,  pady=2,padx=15)
        self.emailfield = Entry(labelframe)
        self.emailfield.grid(row=2, column=2, sticky=W, padx=5, pady=2)
        Label(labelframe, text='Work Done:',bg="grey",fg="white").grid(row=3, column=1, sticky=W,  pady=2,padx=15)
    ...
    def create_tree_view(self):
        ...
        self.tree.heading('#0', text='Patient Name', anchor=W)
        self.tree.heading("email", text='Phone Number', anchor=W)
        self.tree.heading("number", text='Work Done', anchor=W)
